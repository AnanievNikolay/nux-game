...
// @Success 200 {object} dto.Games
// @Failure 500 {string} string
// @Failure 400 {string} string
// @Failure 404 {string} string
// @Router /game/{token}/history [GET]
func (h *Handler) History(c echo.Context) error {
	...
	mf := utils.LogTimeSpent(requestLogger, "History")
	defer mf()
	...
	history, err := h.service.GetHistoryByToken(ctx, requestLogger, token)
	if err != nil {
		requestLogger.Errorf("service.GetHistoryByToken: %s", err)
	...
	return c.JSON(http.StatusOK, dto.NewGames(history))
}
